...
import kornia
from utils.summary_util import *
from config.option import TestOptions
from data.out_dataset import Out_Set
from model.total_model.smodel import parsing
import os
from tools.color import Normalize
...
def apply_tranform(tensors):
    ...
    y_trans_arr.append(kornia.geometry.Resize((opt.input_size, opt.input_size)))
...
def evluate_outdata():
    ...
    model_our = parsing(opt)
    device = "cuda:0"
    model_our = load_model_check(opt.checkpoint_dir, model_our, device)
    dataset = Out_Set(opt, opt.data_dir, b_checkpoint_path=opt.bisenet_dir)
