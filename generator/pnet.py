...
class SResBlock(nn.Module):
    def __init__(self, fin, fout, ref_nc, relu_type, norm_type='spade'):
        ...
        self.norm_0 = SNorm(fmiddle, ref_nc, norm_type)
        self.norm_1 = SNorm(fmiddle, ref_nc, norm_type)
...
class PSFRGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, in_size=512, out_size=512, min_feat_size=16, ngf=64, n_blocks=9, parse_ch=19, relu_type='relu',
        ...
        head = [
                ...
                SResBlock(head_ch, head_ch, ref_ch, relu_type, norm_type),
                ]
        ...
        for i in range(up_steps):
            ...
            body += [
                    nn.Sequential(
                        ...
                        SResBlock(cout, cout, ref_ch, relu_type, norm_type)
                        )
                    ]
            head_ch = head_ch // 2
    ...
    def forward_spade_m(self, m, x, ref):
        if isinstance(m, SNorm) or isinstance(m, SResBlock):
           x = m(x, ref)
        else:
           x = m(x)
        return x
